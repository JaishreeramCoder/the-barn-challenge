...
#   script_path = join(controller_path, 'scripts', 'test_transformer.py')
    ...
    if args.world_idx >= 300:  # DynaBARN environment which does not have a planned path
        path_length = compute_distance(INIT_POSITION[:2], [INIT_POSITION[0] + GOAL_POSITION[0], INIT_POSITION[1] + GOAL_POSITION[1]])
    else:
    ...
    timeout = int((curr_time - start_time) >= 100)
    with open(args.out, "a") as f:
        f.write(f"{args.world_idx} {int(success)} {int(collided)} {timeout} {curr_time - start_time:.4f} {nav_metric:.4f}\n")
